...
生产运行需要已构建的包与前端产物。请在仓库根目录单独运行 `pnpm run build`，然后使用 `pnpm dsh <args...>` 运行该构建入口并转发所有参数；`pnpm run dsh:source` 通过 tsx 运行 TypeScript 入口，仅用于启动器自身的工作，模块解析约定以[源码执行参考](reference/README.zh.md#source-execution)为准。
