...
Inspect the V3 persistence types captured at `dsh-v0.1.5-alpha.1`. The rendered system prompt becomes a `system/message` surface node, request headers no longer carry `system`, and surface replacement endpoints are named `startSeq` / `endSeq`. This snapshot retains V3 declarations before the V3-to-V4 peer-message edge.
...
- `SessionHeader.version` resolves to the literal 3. The logical and physical headers require `isSeeded`; the optional `backend` field records `dsh` or `pi`.
- The rendered system prompt is the surface event `system/message`, and `request/header.data.header.system` is retired.
- Surface replacement endpoints are `startSeq` / `endSeq`; `assistant/chunk` stays absent and both `assistant/message.data` and log-only `assistant/attempt.data` keep an embedded `stream`.
- The PTC vocabulary is `tool/ptc-dispatch-start` / `tool/ptc-dispatch`, and `agent-preset/selected` names the `ptc` preset.
- The snapshot contains 62 roots and 482 reachable types.
...
The [complete machine inventory](v3.schema.json) contains 62 roots and 482 reachable types. Digests include all referenced fields; source names and paths describe the selected historical tree.
...
Sources: `packages/xingchen/xingchen/src/types.ts`
...
| `backend` | optional | [`JsonlHeaderLine.backend`](#persistence-type-jsonlheaderlinebackend) |
...
The [complete machine inventory](v3.schema.json) holds the exact declarations. `pnpm run verify-persistence-formats` re-derives these facts from the archived schema and the checked-in documents; `--write` refreshes the generated regions. Opaque payload contents and behavior that changed no declared type are outside this reference.
