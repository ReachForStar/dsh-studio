---
description: "完整的会话 V3 持久化类型、来源证据与物理编码参考。"
...
查看在 `dsh-v0.1.5-alpha.1` 捕获的 V3 持久化类型。渲染后的系统提示词成为 `system/message` surface 节点，请求头不再携带 `system`，surface 替换端点改名为 `startSeq` / `endSeq`。本快照保留 V3 的声明，早于 V3 到 V4 的对等端消息边。
...
来源是已发布标签 `dsh-v0.1.5-alpha.1`，其写入器常量为 3。[标签记录](../releases/dsh-v0.1.5-alpha.1.zh.md)描述相邻发布的变更。这份完整清单既包含未变的根，也包含该标签下发生变化的根。

提取出的声明来自 `packages/core/session/src/types.ts`、`packages/session/session-persistence-jsonl/src/format.ts`，以及每个第一方 `SessionEventMap` 声明合并及其可达类型。下文中的源码位置指向那棵历史源码树，而非当前文件。

<a id="characteristics"></a>
## 格式特征

以下事实描述所选检查点。下面的生成表格保留精确的字段类型与可选性。

- `SessionHeader.version` 解析为字面量 3。逻辑头与物理头都要求 `isSeeded`；可选字段 `backend` 记录 `dsh` 或 `pi`。
- 渲染后的系统提示词是 surface 事件 `system/message`，`request/header.data.header.system` 已退役。
- surface 替换端点为 `startSeq` / `endSeq`；`assistant/chunk` 仍然缺席，`assistant/message.data` 与仅日志的 `assistant/attempt.data` 都保留内嵌 `stream`。
- PTC 词汇是 `tool/ptc-dispatch-start` / `tool/ptc-dispatch`，`agent-preset/selected` 命名 `ptc` 预设。
- 本快照包含 62 个根与 482 个可达类型。
...
roots:
  "SessionHeader": 8437839e5765b99078a718d8835c732e5ecb84ecfb11653b6f93d5d2a1187d06
  "JsonlHeaderLine": 304e47d57244dcff41c96e5e1dd31f21f818a799a7ff5354017944975e1bec54
  ...
  "event:xingchen/dispatch-progress": 9474bbb067c7b190f6aef17f0afaa7ca30daeaca1dc9539c040fc8e886fa3b01
```
...
[完整机器目录](v3.schema.json)包含 62 个根类型和 482 种可达类型。摘要包含所有引用字段；源码名称和路径描述所选的历史源码树。

### 持久化类型指纹

| 根类型 | 类别 | SHA-256 | 已解析类型 |
|---|---|---|---|
| `SessionHeader` | header | `8437839e5765b99078a718d8835c732e5ecb84ecfb11653b6f93d5d2a1187d06` | [`packages/core/session/src/types.ts#SessionHeader`](#persistence-type-packagescoresessionsrctypestssessionheader) |
| `JsonlHeaderLine` | header | `304e47d57244dcff41c96e5e1dd31f21f818a799a7ff5354017944975e1bec54` | [`packages/session/session-persistence-jsonl/src/format.ts#HeaderLine`](#persistence-type-packagessessionsession-persistence-jsonlsrcformattsheaderline) |
...
来源：`packages/deliverables/tool-present/src/types.ts`
...
来源：`packages/compaction/compaction-image-offload/src/projection.ts`
...
来源：`packages/core/session/src/types.ts` · `packages/core/tools/src/types.ts`
...
来源：`packages/core/session/src/types.ts` · `packages/deliverables/workspace-changes/src/types.ts`
...
| 属性 | 存在性 | 类型 |
|---|---|---|
| `data` | 必需 | [`event:xingchen/dispatch-progress.data`](#persistence-type-eventxingchendispatch-progressdata) |
| `ignorable` | 可选 | `true` |
| `seq` | 必需 | `number` |
| `time` | 必需 | `number` |
...
来源：`packages/xingchen/xingchen/src/types.ts`
...
| `callId` | 可选 | `string` |
| `commandId` | 可选 | `string` |
| `mode` | 必需 | [`event:xingchen/dispatch-progress.data.mode`](#persistence-type-eventxingchendispatch-progressdatamode) |
| `role` | 必需 | [`packages/xingchen/xingchen/src/types.ts#XingchenSpecialistId`](#persistence-type-packagesxingchenxingchensrctypestsxingchenspecialistid) |
| `skill` | 必需 | `string` |
| `state` | 可选 | `string` |
| `text` | 必需 | `string` |
...
<a id="persistence-type-jsonlheaderlinebackend"></a>

#### `JsonlHeaderLine.backend`
...
SHA-256: `8437839e5765b99078a718d8835c732e5ecb84ecfb11653b6f93d5d2a1187d06`
...
| `backend` | 可选 | [`JsonlHeaderLine.backend`](#persistence-type-jsonlheaderlinebackend) |
| `createdAt` | 必需 | `number` |
| `cwd` | 可选 | `string` |
...
来源：`packages/feedback/command-feedback/src/types.ts`
...
来源：`packages/subagent/subagent/src/catalog.ts`
...
[完整机器清单](v3.schema.json)保存精确声明。`pnpm run verify-persistence-formats` 会从已归档的模式与检入的文档重新推导这些事实；`--write` 刷新生成区域。不透明载荷的内部内容、以及未改变已声明类型的行为变更，不在本参考范围内。
