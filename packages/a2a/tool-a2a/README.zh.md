...
`a2a_peers` 返回每个已配置对等端的名字、端点，以及其已发布卡片上的显示名；卡片读不到的对等端会连错误一起列出，而不是让整个列表失败。`a2a_send` 接收对等端名字、文本消息，以及可选的 `contextId` 与 `taskId` 用来续接先前的工作；它返回对方的回复文本，外加后续调用需要的标识。对等端失败时以「失败的工具结果 + 原因」呈现，模型可以据此纠正调用，而不是丢掉这一轮。
...
两个工具定义，各自带描述与 JSON schema：`a2a_peers` 无参数，`a2a_send` 接收 `peer`、`message` 与可选的 `contextId` / `taskId`。两份描述都说明调用做什么、结果意味着什么；对等端列表本身以工具输出到达模型，而不是提示文本。
