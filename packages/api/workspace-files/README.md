...
Reads through `ctx.fs` use the backend's read authority; the sandboxing backend fences writes and edits, not reads. `write` replaces one file's text and `writeBytes` its bytes; both stay inside the Session's workspace, apply the same `maxFileBytes` cap, and carry the version the editor read as a guard, so a file that changed underneath fails with `workspace-file/stale-version` instead of being overwritten. `delete` deletes one entry addressed by path — a file, a symbolic link, or, under `recursive`, a directory with its contents — and reports what it removed; containment is proved on the entry's parent, so a link goes as the link, and a non-empty directory without `recursive` fails with `workspace-file/not-empty`. A backend that cannot store binary content answers `workspace-file/binary-unsupported` rather than writing a decoded file. A Typert lookup derives `WorkspaceFileScope` from a live Session header or the persistence service's header-only `stat`, so cold subagent Sessions need neither Agent activation nor event-body reads. The service adds regular-file checks and bounded transfer, while workspace containment belongs only to directory listing and change observation. A page is cut from `streamText`, which decodes and rejects non-UTF-8 chunk by chunk: the cutter counts lines before the window without keeping them, admits each in-window segment against the byte cap before buffering it, and returns at the first character past the window. One `stat` before the stream names the version and size the page reports.
