...
7. **A dynamic bundle's factory prologue must not require a Node builtin.** The prologue runs at materialization, so a builtin reaching it — a dependency resolved to its Node entry, such as a package whose default export condition is `node` — loads but never activates; the build preset rejects that emission. Import the dependency's browser entry, or alias the specifier in the package's `clientPlugins` ([ui-polish's crypto shim](ui-polish/tsdown.config.ts) is the builtin case).
8. **Keep the published payload closed.** Every relative runtime import and emitted asset must be covered by `files`; the repository publint pass checks the exact publication view.
