...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'setFactory(factory: AgentFactory, backend: AgentBackend = DEFAULT_AGENT_BACKEND): () => void',
        ...
        parameters: [{ name: 'factory', description: 'the loop-owned factory {@link create}/{@link resume} delegate to.' }, { name: 'backend', description: 'the loop backend this factory serves (default `dsh`).' }],
    ...
    methods: [
      {
        ...
        description: 'Read the current registry contents (frozen snapshots; never mutate).',
        parameters: [],
        returns: 'every saved definition in registry order.',
      },
      {
        signature: 'get(ref: SshConnectionId | string): SshConnectionDefinition | undefined',
        description: 'Look one definition up by id or unique name.',
        parameters: [{ name: 'ref', description: 'the id or name to find.' }],
        returns: 'the definition, or undefined when no connection matches.',
      },
      {
        signature: 'async save(input: unknown): Promise<SshConnectionDefinition>',
        description: 'Save one definition: an input carrying `id` updates that connection (which must exist), otherwise a new connection is created. Names are unique across the registry. An update whose auth omits a secret field (password, privateKeyPath, passphrase) inherits the stored value, so write-only callers can never wipe a secret they never saw.',
        parameters: [{ name: 'input', description: 'untrusted save input (tool and wire payloads included).' }],
        returns: 'the normalized, persisted definition.',
      },
      {
        signature: 'async remove(ref: SshConnectionId | string): Promise<boolean>',
        description: 'Remove one connection by id or name.',
        parameters: [{ name: 'ref', description: 'the id or name to remove.' }],
        returns: 'whether a connection was removed.',
      },
      {
        signature: 'knownHostFingerprint(hostPort: string): string | undefined',
        description: 'Read the remembered host key fingerprint of one `host:port` endpoint.',
        parameters: [{ name: 'hostPort', description: 'the endpoint key (e.g. `example.com:22`).' }],
        returns: 'the remembered `SHA256:<base64>` fingerprint, or undefined.',
      },
      {
        signature: 'async rememberHostKey(hostPort: string, fingerprint: string): Promise<void>',
        description: 'Persist a host key fingerprint for one `host:port` endpoint (the accept-new side of host key verification).',
        parameters: [{ name: 'hostPort', description: 'the endpoint key (e.g. `example.com:22`).' }, { name: 'fingerprint', description: 'the `SHA256:<base64>` fingerprint to remember.' }],
      },
      {
        signature: 'abstract connect(id: SshConnectionId): Promise<SshConnection>',
        description: 'Open (or reuse) the shared connection for one definition id. Handles stay open until close or provider teardown.',
        parameters: [{ name: 'id', description: 'the definition id to connect.' }],
        returns: 'the live connection handle.',
      },
      {
        signature: 'abstract close(id: SshConnectionId): Promise<void>',
        description: 'Close and evict the shared connection for one definition id.',
        parameters: [{ name: 'id', description: 'the definition id to disconnect; unknown ids are a no-op.' }],
      },
      {
        signature: 'abstract resolveExec(request: SshExecRequest): SshExecSpec',
        description: 'Apply implementation-owned defaults and caps to an exec request.',
        parameters: [{ name: 'request', description: 'the caller\'s request; omitted fields get this implementation\'s defaults, capped fields are clamped.' }],
        returns: 'the fully-specified spec to hand to {@link SshConnection.exec}.',
      },
      {
        signature: 'async test(ref: SshConnectionId | string): Promise<SshTestResult>',
        description: 'Verify that a connection definition can be reached: open its shared connection, run a probe command, and close it again. The close evicts the shared handle, so a concurrent user reconnects on its next call.',
        parameters: [{ name: 'ref', description: 'the id or name to test.' }],
        returns: 'the successful probe with its round-trip latency.',
        throws: ['{@link SshError} with `SSH_NOT_FOUND` or `SSH_CONNECT_FAILED`.'],
      },
      {
        signature: 'toView(definition: SshConnectionDefinition): SshDefinitionView',
        description: 'Project one definition to its secret-free wire view.',
        parameters: [{ name: 'definition', description: 'the definition to project.' }],
        returns: 'the secret-free view for wire surfaces.',
      },
      {
        signature: 'resolve(ref: SshConnectionId | string): SshConnectionDefinition',
        description: 'Resolve a caller reference (id or unique name) against the registry.',
        parameters: [{ name: 'ref', description: 'the id or name to find.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'AgentBackend',
    declaration: 'export type AgentBackend = \'dsh\' | \'pi\';',
  },
  {
  ...
  },
  {
    ...
    declaration: 'export interface SshConnectionDefinition {\n    id: SshConnectionId;\n    name: string;\n    host: string;\n    port: number;\n    username: string;\n    auth: SshAuth;\n    connectTimeoutMs: number;\n    hostKeyFingerprint?: string | undefined;\n}',
  },
  {
  ...
  {
    name: 'SshExecRequest',
    declaration: 'export interface SshExecRequest {\n    command: string;\n    timeoutMs?: number;\n    cwd?: string;\n    signal?: AbortSignal;\n}',
  },
  {
    name: 'SshExecSpec',
