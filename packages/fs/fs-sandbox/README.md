...
| File | Role |
|---|---|
| [`src/index.ts`](src/index.ts) | `SandboxedFileSystem`: mode fence on `writeText`/`writeBytes`/`editText`/`remove`, `sandboxMode` fact |
