...
The same reading serves requests. A route that lists no models and that the installed catalog does not describe is read once per configuration, and the models it answers with become that route's catalog for every operation until the configuration changes. Only a route whose endpoint has a listing this build can read qualifies — `openai-completions`, `openai-responses`, or `anthropic-messages`, whether the route names the protocol or the installed catalog card supplies it; a route with no endpoint, or with one this build cannot read, still needs its models listed. The route's `timeoutMs` bounds the reading when it is set, and a reading that fails leaves that route's requests failing with the endpoint's own fault — an unreachable URL, a rejected key — until the configuration changes.

### Failures and recovery

A route pi-ai does not ship needs `api`, `baseURL`, and models — listed in configuration or read from its endpoint; a route with neither is refused where it is written, naming the route and model. Failures carry stable codes: a credential that cannot be used fails with `INVALID_CREDENTIAL` naming the route and reference, a route whose `apiKeyEnv` reference resolves to nothing fails with `MISSING_CREDENTIAL`, an unconfigured model fails with `UNKNOWN_MODEL`, and terminal provider failures distinguish `QUOTA` from transient `RATE_LIMIT`. `GenerateOptions.stop` is rejected with `UNSUPPORTED_OPTION` because pi-ai's common streaming UI cannot guarantee it across providers.
...
| [`src/discovery.ts`](src/discovery.ts) | Endpoint interrogation, and the runtime catalog an endpoint-served route reads for itself |
...
- **A catalog read from an endpoint is read once per configuration** — the reading happens on the first operation that needs that route, is reused for the configuration's lifetime, and repeats only after a configuration change or a restart; a model the endpoint adds meanwhile is not noticed until then. A route that lists its own models is never read.
