...
- **Sandbox fence.** The per-call policy decides: `read-only` denies, `danger-full-access` permits, and `workspace-write` requires the canonical target under a `writableRoots()` entry; reads pass through.
