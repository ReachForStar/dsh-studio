...
export function assertReleasedV3Header(header: SessionFormatHeader): void {
  ...
  // shape is validated without it, then the value is bounded here.
  const { backend, ...v2Fields } = header
  assertReleasedV2Header({ ...v2Fields, version: 2 })
  if (backend !== undefined && backend !== 'dsh' && backend !== 'pi') {
    throw new SessionFormatError('format v3 header backend must be "dsh" or "pi"')
  }
}
