/** Durable composition of historical chunk collapse and current system/reference migration. */
...
describe.each([0, 1] as const)('V%s multi-edge durable publication', (version) => {
  it.each(['none', 'zstd'] as const)('publishes only V4 after chunk collapse, system changes, and reference remapping (%s)', async (compression) => {
  ...
  it.each(['none', 'zstd'] as const)('refuses a late current prompt outside a step without publishing earlier edges (%s)', async (compression) => {
