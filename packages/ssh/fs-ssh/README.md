...
Reads preserve the shared filesystem error codes. Writes, edits, and removals send the resolved per-call policy to the helper, which canonicalizes the workspace and enforces it beside the mutation. Lost transport reports an I/O failure; a mutation may already have committed and is not retried automatically.
