...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      // The fork keeps product packages under the @reachforstar scope; both
      // scopes are publishable members of a family.
      if (!name.startsWith('@deepseek-ai/') && !name.startsWith('@reachforstar/')) {
        throw new Error(`${normalized} must name an @deepseek-ai or @reachforstar package`)
      }
